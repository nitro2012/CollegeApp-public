...
import com.example.collegeapp.Faculty.AddClass;
import com.example.collegeapp.Faculty.ClassAdapter;

import com.example.collegeapp.Faculty.FacultyPanel;
import com.example.collegeapp.Faculty.TeacherData;
import com.example.collegeapp.Faculty.classesData;
...
public class StudentPanel extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        reference2= Utils.getDatabase().getReference().child("Classes");
        reference= Utils.getDatabase().getReference().child("Student").child(userId);

        studentReference = Utils.getDatabase().getReference().child("Student");
    ...
    private void updateClass() {
        ...
        reference2.child(mClass).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull @NotNull DataSnapshot snapshot) {
                ...
                classesData post = new classesData(cd.getClassName(),cd.getSection(),cd.getTeacherName(), cd.getClassId(), studentArr,fdat);
                Map<String, Object> postValues = post.toMap();
